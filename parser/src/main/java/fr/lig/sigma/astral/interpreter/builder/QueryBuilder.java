...
public class QueryBuilder {
    //private QueryVisitor<Entity, Entity> ob;
    ...
    public void induceSource(Document doc) throws SourceAlreadyExistsException, InstanceCreationException, UnknownSourceException {
        ...
        if (sourceNode == null)
            sourceNode = appendSourceNode(doc);
    ...
    private void fetchDeclaredSource(Document doc, Node sourceNode, String id, Element queryElement) throws UnknownSourceException {
        ...
        for (Map.Entry<String, Object> entry : prop.entrySet()) {
            ...
            if (value != null)
                newSource.setAttribute(entry.getKey(), value.toString());
        }
    ...
    private Properties extractProperties(Element e, boolean changeId) {
        ...
        if (children.getLength() > 0) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                if ("filter".equals(elt.getNodeName()))
                    filter.put(elt.getAttribute("name"), elt.getAttribute("value"));
                if ("argument".equals(elt.getNodeName()))
                    arguments.put(elt.getAttribute("name"), elt.getAttribute("value"));
            }
            //if (!filter.isEmpty())
        ...
        for (int i = 0; i < e.getAttributes().getLength(); i++) {
            ...
            if (changeId && "id".equals(t.getName()))
                p.put("entityname", t.getValue());
            else
                p.put(t.getName(), t.getValue());
        }
        return p;
    }
