 ...
 *
 */
public class BlockedStreamSource extends StreamContainer implements Source {
    protected SourceBlocker blocker;
    public BlockedStreamSource() {
    }
    ...
    public void putAll(TupleSet ts, int i) throws AxiomNotVerifiedException {
        super.putAll(ts, i);
        if(ts.size() > 0)
            blocker.release(new Batch(ts.iterator().next().getTimestamp(), i));
    }

    @Override
    public void put(Tuple t, int i) throws AxiomNotVerifiedException {
        super.put(t, i);
        blocker.release(new Batch(t.getTimestamp(), i));
    }

    @Override
    public void firstSchedule() throws Exception {
        blocker = new SourceBlocker(getScheduler()); //getScheduler().getBlocker();
