...
@Component
@Provides
public class UnaryRelationOperator extends RelationOperator implements Operator {
    ...
    public static void computeOperations(TupleSet current, TupleSet resultSet, List<UnaryRelationalOperation> unaryOperations) {
        for (Tuple t : current) {
            Tuple res = t;
    ...
    public void processEvent(Batch b) throws AxiomNotVerifiedException {
        ...
        TupleSet resultSet = entityFactory.instanciateTupleSet(getAttributes());
        computeOperations(current, resultSet, unaryOperations);
        update(resultSet, b);
    }
