...
public class PartitionedWindowImpl extends RelationOperator implements Operator {
        ...
        while (in.hasNext(timestamp)) {
            ...
            if (dest == null) {
                ...
                try {
                    ...
                    //win.setWindow(dest, (WindowDescription) desc.clone());
                    // TODO Bad structure
                    windows.add(win);
            ...
            if (log.isTraceEnabled())
                log.trace(timestamp + ": New tuple inside the substream (values " + values + ")");
        }
    }
    ...
    public void prepare() throws Exception {
        ...
        setOutput((Relation) entityFactory.instanciateEntity("RelationVolatileImpl",
                in.getName() + "[" + partitionedBy + "/" + description.get(0) + "]", in.getAttributes()));
        addInput(in, true);
