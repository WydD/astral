...
public class SymetricHashJoin extends DynamicRelationOperator implements Operator {
    ...
    public void prepare() throws Exception {
        ...
        if (optional != null)
            otherAttributes.removeAll(conditionAttributes);


        if (condition != null && !condition.isEmpty())
            this.optional = engine.getGlobalEA().buildCondition(condition);

        if (optional != null && !attributes.containsAll(conditionAttributes))
            throw new WrongAttributeException("Condition " + optional + " has attributes that does not match " + attributes);
    ...
    public void processEvent(Batch b) throws AxiomNotVerifiedException {
        ...
        if (log.isTraceEnabled())
            log.trace(b + ": Join (on " + joinAttributes + ") computed with a size of " + getSizeAsString(is, 0, ds) +
                    " (comparisons: " + comparisons + ", " + (defaultOuter != null ? "outer: " + outer + ", " : "") +
                    "left: " + getSizeAsString(isLeft, leftContent.size(), dsLeft) + ", right: " + getSizeAsString(isRight, rightContent.size(), dsRight) + ")");
    }
